...
public class BlockEmergencyLamp extends BlockIEnTileProvider<BlockTypes_EmergencyLamp> {
	...
	{
		if (getMetaFromState(state) == BlockTypes_EmergencyLamp.SOLAR_PANEL.getMeta())
		{
    ...
    public boolean canIEBlockBePlaced(World world, BlockPos pos, IBlockState newState, EnumFacing side, float hitX, float hitY, float hitZ, EntityPlayer player, ItemStack stack) {
        ...
        if (stack.getItemDamage() == BlockTypes_EmergencyLamp.EMERGENCY_LAMP.getMeta())
    ...
    public void neighborChanged(IBlockState state, World world, BlockPos pos, Block block, BlockPos fromPos) {
        ...
        if (tile instanceof TileEntitySolarPanel) {
            ...
            if (!(blockBelow.isTopSolid(downState) || downState.getBlockFaceShape(world, pos.down(), EnumFacing.UP) == BlockFaceShape.SOLID || downState.getBlock().isOpaqueCube(downState))) {
                this.dropBlockAsItem(world, pos, IEnContent.blockGenerators0.getStateFromMeta(BlockTypes_EmergencyLamp.EMERGENCY_LAMP.getMeta()), 0);
    ...
    @Override
    public TileEntity createBasicTE(World worldIn, BlockTypes_EmergencyLamp type) {
        switch (type) {
            case EMERGENCY_LAMP:
    ...
    public EnumBlockRenderType getRenderType(IBlockState state) {
        if (getMetaFromState(state) == BlockTypes_EmergencyLamp.EMERGENCY_LAMP.getMeta()) {
