...
public class BlockIEnSlab<E extends Enum<E> & BlockIEnBase.IBlockEnum> extends BlockIEnTileProvider<E>
{
	public static final PropertyInteger prop_SlabType = PropertyInteger.create("slabtype", 0, 2);

	public BlockIEnSlab(String name, Material material, PropertyEnum<E> property)
	{
		super(name, material, property, ItemBlockIEnSlabs.class, prop_SlabType);
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess world, BlockPos pos)
	{
		...
		if(tile instanceof TileEntityIEnSlab)
			return state.withProperty(prop_SlabType, ((TileEntityIEnSlab)tile).slabType);
		return state;
	}
	//	@Override
	...
	{
		if(tile instanceof TileEntityIEnSlab&&!player.capabilities.isCreativeMode)
		{
			spawnAsEntity(world, pos, new ItemStack(this, ((TileEntityIEnSlab)tile).slabType==2?2: 1, this.getMetaFromState(state)));
			return;
		}
	...
	public boolean isSideSolid(IBlockState state, IBlockAccess world, BlockPos pos, EnumFacing side)
	{
		...
		if(te instanceof TileEntityIEnSlab)
		{
			int type = ((TileEntityIEnSlab)te).slabType;
			if(type==0)
