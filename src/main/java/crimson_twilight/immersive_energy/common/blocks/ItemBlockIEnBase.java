...
public class ItemBlockIEnBase extends ItemBlock
{
	public ItemBlockIEnBase(Block b)
	{
		super(b);
		if(((BlockIEnBase)b).enumValues.length>1)
			setHasSubtypes(true);
	}
	...
	{
		return ((BlockIEnBase) this.block).getUnlocalizedName(stack);
	}

	@Override
	public void addInformation(ItemStack stack, World worldIn, List<String> list, ITooltipFlag advInfo)
	{
		if(((BlockIEnBase)block).hasFlavour(stack))
		{
			String subName = ((BlockIEnBase)this.block).getStateFromMeta(stack.getItemDamage()).getValue(((BlockIEnBase)this.block).property).toString().toLowerCase(Locale.US);
			String flavourKey = "desc." + ImmersiveEnergy.MODID + ".flavor." + ((BlockIEnBase)this.block).name+"."+subName;
	...
	{
		if(!((BlockIEnBase)this.block).canIEBlockBePlaced(world, pos, newState, side, hitX,hitY,hitZ, player, stack))
			return false;
		boolean ret = super.placeBlockAt(stack, player, world, pos, side, hitX, hitY, hitZ, newState);
		if(ret)
		{
			((BlockIEnBase)this.block).onIEBlockPlacedBy(world, pos, newState, side, hitX,hitY,hitZ, player, stack);
		}
		return ret;
	}
	@Override
	...
	{
		BlockIEnBase blockIn = (BlockIEnBase) this.block;
