...
{
	public ItemBlockIEnSlabs(Block b)
	{
		super(b);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		...
		TileEntityIEnSlab stackSlab = null;
		if(side.getAxis().isVertical()&&this.block.equals(world.getBlockState(posThere).getBlock())&&world.getBlockState(posThere).getBlock().getMetaFromState(world.getBlockState(posThere))==stack.getItemDamage())
		{
			...
			if(te instanceof TileEntityIEnSlab&&((TileEntityIEnSlab)te).slabType+side.ordinal()==1)
				stackSlab = ((TileEntityIEnSlab)te);
		}
	...
	public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ, IBlockState newState)
	{
		...
		if(ret)
		{
			...
			if(tileEntity instanceof TileEntityIEnSlab)
				((TileEntityIEnSlab)tileEntity).slabType = (side==EnumFacing.DOWN||(side!=EnumFacing.UP&&hitY >= .5))?1: 0;
		}
		return ret;
	}
}
