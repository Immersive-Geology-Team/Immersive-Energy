...
public class IENBlockStateProvider extends BlockStateProvider
{
    ...
    protected Logger logger = IENLib.getNewLogger();

    public IENBlockStateProvider(DataGenerator generator, ExistingFileHelper helper)
    {
        super(generator.getPackOutput(), IENLib.MODID, helper);
    ...
    private void generateGenericBlock(Block block, String block_name, String block_parent)
    {
        getVariantBuilder(block).forAllStates(state -> ConfiguredModel.builder().modelFile(models().withExistingParent(
                new ResourceLocation(IENLib.MODID, "block/" + block_name).getPath(),
                new ResourceLocation(IENLib.MODID, "block/" + block_parent))).build());
    }

    private void genericmultiblock(String registry_name)
    {
        IENLib.IEN_LOGGER.info("Generating ["+ registry_name +"] Multiblock Model Data");
        createMultiblock(innerObj("block/multiblock/obj/"+registry_name+"/"+registry_name+".obj"), IENRegistrationHolder.getMBTemplate.apply(registry_name));
    }

    private void genericmultiblockMirror(String registry_name)
    {
        IENLib.IEN_LOGGER.info("Generating ["+ registry_name +"] with Custom Mirror Multiblock Model Data");
        testCreateMultiblock(innerObj("block/multiblock/obj/"+registry_name+"/"+registry_name+".obj"), innerObj("block/multiblock/obj/"+registry_name+"/"+registry_name+"_mirrored.obj"),  (IENTemplateMultiblock) IENRegistrationHolder.getMBTemplate.apply(registry_name), false);
    }

    private void createMultiblock(NongeneratedModel unsplitModel, TemplateMultiblock multiblock)
    {
        createMultiblock(unsplitModel, (IENTemplateMultiblock) multiblock, false);
    }

    private void createMultiblock(NongeneratedModel unsplitModel, IENTemplateMultiblock multiblock, boolean dynamic)
    {
    ...
    private void testCreateMultiblock(NongeneratedModel unsplitModel, NongeneratedModel mirror_model, IENTemplateMultiblock multiblock, boolean dynamic)
    {
        final ModelFile mainModel = split(unsplitModel, multiblock, false, dynamic);
    ...
    private ModelFile split(NongeneratedModel loc, IENTemplateMultiblock mb)
    {
        return split(loc, mb, false);
    }

    private ModelFile split(NongeneratedModel loc, IENTemplateMultiblock mb, boolean mirror)
    {
        return split(loc, mb, mirror, false);
    }

    private ModelFile split(NongeneratedModel loc, IENTemplateMultiblock mb, boolean mirror, boolean dynamic)
    {
    ...
    private ModelFile split(
            NongeneratedModel name, IENTemplateMultiblock multiblock, UnaryOperator<BlockPos> transform, boolean dynamic
    )
    {
